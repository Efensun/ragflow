...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler('./clickup_sync.log'),
...
def check_processing_status():
    ...
    try:
        ...
        for folder_id, folder_name in folder_configs:
            ...
            logger.info(f"{folder_name}: {len(files)} 文件, {processing_count} 处理中, {completed_count} 已完成, {failed_count} 失败")
        
        logger.info(f"总计: {total_files} 文件, {total_processing} 处理中, {total_completed} 已完成, {total_failed} 失败")
